  ...
  class SlotGames
    ...
    def call
      return Array.wrap(stats(game: game)) if game.present?
       ...
       SlotGame.find_in_batches do |group|
    ...
    def game_transactions(game:)
      return game.session_transactions.slot_game unless start_date && end_date

      game.session_transactions.slot_game.between(start_date, end_date)
    end
  end
end
