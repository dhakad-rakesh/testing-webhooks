class TopCasinoFilter
  ...
  def stats
    @stats ||= top_casino_game_ids.map { |game_id| casino_game_stats(game_id: game_id) }
  end

  private

  def top_casino_game_ids
    CasinoItem.all.map { |game| [game.id, pl(game: game)] }
                  .sort_by { |_, profit| -profit }
                  .first(5)
                  .map(&:first)
  end

  def casino_game_stats(game_id:)
    game = CasinoItem.find_by(id: game_id)
