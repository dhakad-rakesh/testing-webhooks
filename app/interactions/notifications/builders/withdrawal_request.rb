module Notifications
  module Builders
    class WithdrawalRequest < ApplicationInteraction
      ...
      def title
        key = 'messages.notifications.withdrawal_request.title'
        localized_message(key, specifiers)
      end

      def body
        key = 'messages.notifications.withdrawal_request.body'
        localized_message(key, specifiers)
      end
      ...
      def click_action
        if ledger.pending?
          Rails.application.routes.url_helpers.admin_withdrawals_url
        else
