class Api::Lsports::ScheduleMatchController < ActionController::Base
  before_action :authenticate_request

  def get_fixtures
    begin
      sport_name = params[:sport_name]
      file = "/tmp/ls_#{sport_name}_fixtures.json.gz"
  ...
  def get_fixture_markets
    begin
      sport_name = params[:sport_name]
      file = "/tmp/ls_#{sport_name}_fixture_market.json.gz"
      send_file file, status: :ok
    rescue StandardError => e
      render status: 404
    end
