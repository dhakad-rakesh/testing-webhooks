...
function getDashboardData(data = {}, interval = true) {
  ...
  getAjaxLoad('/admin/dashboard/casino_overview_filter', data, '#casino_overview_filter');
  getAjaxLoad('/admin/dashboard/slot_game_overview_filter', data, '#slot_game_overview_filter');
  getAjaxLoad('/admin/dashboard/top_sports_filter', data);
  getAjaxLoad('/admin/dashboard/top_casino_filter', data);
  getAjaxLoad('/admin/dashboard/top_slot_game_filter', data);
}
...
function resetProfitData() {
  ...
  $('#bet-casino .data').attr('data-current', 0);
  $('#win-casino .data').attr('data-current', 0);
  $('#bet-slot .data').attr('data-current', 0);
  $('#win-slot .data').attr('data-current', 0);
}
...
function updateProfitComponent(selector, newProfit) {
  ...
  selector.html(`<div class='currency-icon'>${newProfit}<img src='/assets/eth-icon.png'</div>`);
...
function updateProfitView(data) {
  ...
  const betCasino = $('#bet-casino .data');
  const winCasino = $('#win-casino .data');
  const betSlot = $('#bet-slot .data');
  const winSlot = $('#win-slot .data');
  ...
  updateProfitComponent(betCasino, data.casino.bet);
  updateProfitComponent(winCasino, data.casino.win);
  updateProfitComponent(betSlot, data.slot.bet);
  updateProfitComponent(winSlot, data.slot.win);
}
